    ...
    <Link href={`/notes/${noteId}`} className="">
      ...
      >
        <CardHeader className="p-0  w-full relative">
          <div className=" absolute right-0 top-[2px]">
            {myNote ? (
              ...
              <FaRegBookmark size={25} className="rounded-lg p-1 hover:bg-[#90b7ff]"/>
            )}
          </div>
        </CardHeader>
        <CardTitle className="m-0 ">
          <span className=" font-semibold text-[18px] px-[1px] rounded-md py-[2px] shadow-inner">{title}</span>
        </CardTitle>
        <div className=" cursor-pointer w-full h-full overflow-hidden  mt-2 text-center font-[400]">
          ...
          <div dangerouslySetInnerHTML={{__html:content}} className="truncate text-[12px] text-wrap leading-tight "/>
        </div>
